...
def main(arg_dict):
    ...
    arg_dict[
        "feature_dims"] = fe.get_feature_dims()  # arg_dict["feature_dims"]: {'player': 29, 'ball': 18, 'left_team': 7, 'left_team_closest': 7, 'right_team': 7, 'right_team_closest': 7}

    model = importlib.import_module("models." + arg_dict["model"])
    cpu_device = torch.device('cpu')
    center_model = model.Model(arg_dict)  # 见最下面模型结构
    ...
    processes = []
    p = mp.Process(target=learner, args=(center_model, data_queue, signal_queue, summary_queue, arg_dict))  # target：调用对象/目标函数  args：调用对象的位置参数元组/按位置给目标函数传参
    p.start()
    processes.append(p)
    for rank in range(arg_dict["num_processes"]):
        if arg_dict["env"] == "11_vs_11_kaggle":
            p = mp.Process(target=actor_self,
                           args=(rank, center_model, data_queue, signal_queue, summary_queue, arg_dict))
        else:
