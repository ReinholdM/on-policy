...
def get_data(queue, arg_dict, model):
    data = []
    for i in range(arg_dict["buffer_size"]):  # buffer_size:6
        mini_batch_np = []
        for j in range(arg_dict["batch_size"]): # batch_size:32
            rollout = queue.get()
...
def learner(center_model, queue, signal_queue, summary_queue, arg_dict):
    ...
    # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu") # rjq cpu
    device = "cpu"
    model = imported_model.RNNAgent(arg_dict, device)
    model.load_state_dict(center_model.state_dict())
    algo = imported_algo.QLearner(arg_dict, model)
    # algo.mixer.load_state_dict(center_mixer.state_dict())

    # if torch.cuda.is_available(): # rjq cpu
    #     algo.cuda()
    ...
    while True:
        if queue.qsize() > arg_dict["batch_size"]*arg_dict["buffer_size"]:  # 32*6 条数据
